...
class SyncServer:
    ...
    def remove_client(self, client):
        with self.lock:
            ...
            if client.session_id == SESSION_ID and client.client_id in self.sessions[SESSION_ID]:
                ...
                # Останавливаем таймеры без отправки RESET
    ...
    def process_found(self, client):
        with self.lock:
            ...
            if len(found_clients) == 1:
                ...
                # Устанавливаем время обнаружения
    ...
    def handle_timeout(self, client):
        with self.lock:
            print(f"Таймаут ожидания второго клиента для {client.client_id}")
            if client.client_id in self.timers:
                del self.timers[client.client_id]
            
            # Отправляем DECLINE только этому клиенту
            client.send_command("DECLINE")
    ...
    def accept_game(self):
        ...
        for client in session_clients:
            if client.state == "found":
                # Проверяем, что игра найдена недавно
                if hasattr(client, 'found_time') and current_time - client.found_time < SYNC_TIMEOUT + 5:
